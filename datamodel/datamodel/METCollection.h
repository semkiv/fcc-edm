...
//  METCollection(METVector* data, int collectionID);
  ...
  const std::array<double,arraysize> Magnitude() const;
  template<size_t arraysize>  
  const std::array<double,arraysize> Phi() const;
  template<size_t arraysize>  
  const std::array<double,arraysize> ScalarSum() const;
...
template<size_t arraysize>
const std::array<double,arraysize> METCollection::Magnitude() const {
...
template<size_t arraysize>
const std::array<double,arraysize> METCollection::Phi() const {
  std::array<double,arraysize> tmp;
  auto valid_size = std::min(arraysize,m_entries.size());
  for (unsigned i = 0; i<valid_size; ++i){
...
template<size_t arraysize>
const std::array<double,arraysize> METCollection::ScalarSum() const {
  std::array<double,arraysize> tmp;
  auto valid_size = std::min(arraysize,m_entries.size());
  for (unsigned i = 0; i<valid_size; ++i){
