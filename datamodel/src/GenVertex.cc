...
GenVertex::GenVertex(fcc::Point Position,double Ctau) : m_obj(new GenVertexObj()){
 m_obj->acquire();
...
GenVertex::operator ConstGenVertex() const {return ConstGenVertex(m_obj);}
  ...
  const double& GenVertex::Ctau() const { return m_obj->data.Ctau; }
...
void GenVertex::Ctau(double value){ m_obj->data.Ctau = value;}
