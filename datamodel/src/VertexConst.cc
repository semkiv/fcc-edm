...
ConstVertex::ConstVertex(double Chi2,unsigned Ndf,fcc::Point Position,unsigned Bits) : m_obj(new VertexObj()){
 m_obj->acquire();
...
ConstVertex::~ConstVertex(){
  ...
  const double& ConstVertex::Chi2() const { return m_obj->data.Chi2; }
