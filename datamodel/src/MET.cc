...
MET::MET(double Magnitude,double Phi,double ScalarSum) : m_obj(new METObj()){
 m_obj->acquire();
...
MET::operator ConstMET() const {return ConstMET(m_obj);}
  ...
  const double& MET::Magnitude() const { return m_obj->data.Magnitude; }
  const double& MET::Phi() const { return m_obj->data.Phi; }
  const double& MET::ScalarSum() const { return m_obj->data.ScalarSum; }

void MET::Magnitude(double value){ m_obj->data.Magnitude = value;}
void MET::Phi(double value){ m_obj->data.Phi = value;}
void MET::ScalarSum(double value){ m_obj->data.ScalarSum = value;}
