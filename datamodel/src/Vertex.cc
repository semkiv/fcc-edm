...
Vertex::Vertex(double Chi2,unsigned Ndf,fcc::Point Position,unsigned Bits) : m_obj(new VertexObj()){
 m_obj->acquire();
...
Vertex::operator ConstVertex() const {return ConstVertex(m_obj);}
  ...
  const double& Vertex::Chi2() const { return m_obj->data.Chi2; }
...
void Vertex::Chi2(double value){ m_obj->data.Chi2 = value;}
