...
ConstMET::ConstMET(double Magnitude,double Phi,double ScalarSum) : m_obj(new METObj()){
 m_obj->acquire();
...
ConstMET::~ConstMET(){
  ...
  const double& ConstMET::Magnitude() const { return m_obj->data.Magnitude; }
  const double& ConstMET::Phi() const { return m_obj->data.Phi; }
  const double& ConstMET::ScalarSum() const { return m_obj->data.ScalarSum; }
